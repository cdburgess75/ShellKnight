...
<#
.SYNOPSIS
    ShellKnight v0.82  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.82  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
# PST on network/cloud and large OST detection
try {
...
# Startup impact report - expanded classification
try {
    $startupEntries = @(Get-CimInstance Win32_StartupCommand -ErrorAction SilentlyContinue)
    if ($startupEntries.Count -gt 0) {
        Log-Info "Startup entries  -  $($startupEntries.Count) found:"

        $highImpact = @(
            'OneDrive','Dropbox','GoogleDrive','Box','Steam','EpicGames','Battle.net',
            'Discord','Spotify','Adobe','Creative Cloud','Teams','Slack','Skype',
            'iTunes','iCloud','Cortana','Xbox','GameBar','Nvidia','GeForce'
        )
        $mediumImpact = @(
            'Chrome','Edge','Zoom','QuickBooks','Acrobat','Reader','Java',
            'Malwarebytes','Avast','AVG','McAfee','Sophos','ESET'
        )
        $suspectPatterns = @('updater','helper','launcher','scheduler','agent','notif','tray','sync')
        ...
        foreach ($s in $startupEntries) {
            ...
            $isHigh = $highImpact | Where-Object { $name -match $_ -or $cmd -match $_ }
            $isMed  = $mediumImpact | Where-Object { $name -match $_ -or $cmd -match $_ }
            $isSusp = $suspectPatterns | Where-Object { $name -match $_ -or $cmd -match $_ }

            if ($isHigh) { $tier = 'HIGH';   $tag = '  [HIGH IMPACT]' }
            elseif ($isMed) { $tier = 'MED'; $tag = '  [MED IMPACT]'  }
            elseif ($isSusp) {               $tag = '  [REVIEW: possible unnecessary startup entry]' }

            Log-Info "  [$tier] $name$tag  -  $cmd"
        }
    } else {
        Log-Info "Startup entries  -  none found"
    }

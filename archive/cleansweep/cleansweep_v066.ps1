...
<#
.SYNOPSIS
    Dave's CleanSweep v0.66  -  Enterprise PUP, Adware & Malware IOC Remediation Tool
...
Write-Host "  Dave's CleanSweep v0.66" -ForegroundColor Cyan
...
# Windows Update cache  -  stop service with extended wait before cleaning
try {
    $wuSvc = Get-Service -Name wuauserv -ErrorAction SilentlyContinue
    if ($wuSvc) {
        if ($wuSvc.Status -ne 'Stopped') {
            ...
            while ($waited -lt 30) {
                ...
                $waited += 2
                $wuSvc.Refresh()
                if ($wuSvc.Status -eq 'Stopped') { break }
            }
        }

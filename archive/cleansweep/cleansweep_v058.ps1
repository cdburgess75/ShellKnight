...
<#
.SYNOPSIS
    Dave's CleanSweep v0.58  -  Enterprise PUP, Adware & Malware IOC Remediation Tool
...
Write-Host "  Dave's CleanSweep v0.58" -ForegroundColor Cyan
...
if ($Script:HasCimSession) {
    try {
        ...
        $ErrorActionPreference = 'Stop'
        $bindings | Where-Object { $_.Filter -notmatch 'SCM|BVT|TSlogon|OfficeSync|SCM Event Log' } |
        ForEach-Object {
...
function Send-CleanSweepReport {
    ...
    try {
        ...
        $msg.To.Add($Script:Config.SmtpTo)
        $msg.Subject    = $subject
        $msg.Body       = $htmlBody
        $msg.IsBodyHtml = $true
        ...
        if (Test-Path -LiteralPath $Script:Config.LogPath) {
            $attachment = New-Object System.Net.Mail.Attachment($Script:Config.LogPath)
            $msg.Attachments.Add($attachment)
        }

        $smtp = New-Object System.Net.Mail.SmtpClient($Script:Config.SmtpServer, $Script:Config.SmtpPort)
        $smtp.EnableSsl   = $Script:Config.SmtpUseTLS
        $smtp.Credentials = New-Object System.Net.NetworkCredential($Script:Config.SmtpUser, $Script:Config.SmtpPass)
        $smtp.Timeout     = 30000
        ...
        Log-Warn "Email send failed  -  $($_.Exception.Message)"
    }
}

# Reload log writer briefly to capture email result, then close again
$Script:LogWriter = New-Object System.IO.StreamWriter($Script:Config.LogPath, $true, [System.Text.Encoding]::UTF8)
$Script:LogWriter.AutoFlush = $true
Send-CleanSweepReport
$Script:LogWriter.Flush()
$Script:LogWriter.Close()
$Script:LogWriter.Dispose()
...
if ($totalIssues -eq 0) {
    ...
    $pad = ' ' * ([Math]::Max(0, 22 - $totalIssues.ToString().Length))
    Write-Host ('  ' + ('#' * 76))                                                                         -ForegroundColor Yellow
    Write-Host ('  #' + (' ' * 74) + '#')                                                                  -ForegroundColor Yellow
    Write-Host "  #   Dave is Sweeping!  -  $totalIssues issue(s) detected. Review report above.$pad#"     -ForegroundColor Yellow
    Write-Host ('  #' + (' ' * 74) + '#')                                                                  -ForegroundColor Yellow
    Write-Host ('  ' + ('#' * 76))                                                                         -ForegroundColor Yellow
}
Write-Host ''

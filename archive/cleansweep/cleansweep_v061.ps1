...
<#
.SYNOPSIS
    Dave's CleanSweep v0.61  -  Enterprise PUP, Adware & Malware IOC Remediation Tool
...
Write-Host "  Dave's CleanSweep v0.61" -ForegroundColor Cyan
...
try {
    ...
    if ($svcEvents.Count -gt 0) {
        $svcEvents | ForEach-Object {
            $evt = $_
            # Extract named fields from event properties  -  avoids "A service was installed" generic message
            # 7045 property order: ServiceName, ImagePath, ServiceType, StartType, AccountName
            $svcName = ''
            $svcPath = ''
            $svcAcct = ''
            try {
            ...
            $svcDetail = "Service: $svcName | Path: $svcPath | Account: $svcAcct"

            if ($Script:Targets.IsMatch("$svcName $svcPath") -or
                ...
                Log-IOC "Event 7045 (Service Install) suspicious  -  Time: $($evt.TimeCreated.ToString('yyyy-MM-dd HH:mm:ss')) | $svcDetail"
            }
        }
...
# ==================================================================================================
Log-Info 'Email report skipped  -  O365 Basic Auth disabled. See log header for re-enable instructions.'
Log-Info "Report saved locally: $($Script:Config.LogPath)"

# Reload log writer briefly to capture above messages, then close again

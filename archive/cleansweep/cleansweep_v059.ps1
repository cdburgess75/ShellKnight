...
<#
.SYNOPSIS
    Dave's CleanSweep v0.59  -  Enterprise PUP, Adware & Malware IOC Remediation Tool
...
Write-Host "  Dave's CleanSweep v0.59" -ForegroundColor Cyan
...
function Send-CleanSweepReport {
    ...
    try {
        ...
        # Copy log to temp file to avoid file-in-use lock on the open StreamWriter
        $logCopyPath = $null
        if (Test-Path -LiteralPath $Script:Config.LogPath) {
            ...
            try {
                ...
                $attachment = New-Object System.Net.Mail.Attachment($logCopyPath)
                $msg.Attachments.Add($attachment)
            } catch { $logCopyPath = $null }
        }

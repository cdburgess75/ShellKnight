...
<#
.SYNOPSIS
    ShellKnight v0.78  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.78  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
# Startup impact report
try {
    $startupApprovedPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run'
    $startupApproved = @(Get-ItemProperty $startupApprovedPath -ErrorAction SilentlyContinue |
    ...
    if ($startupEntries.Count -gt 0) {
        ...
        $knownHeavy = @('OneDrive','Dropbox','GoogleDrive','Spotify','Discord','Slack','Teams',
                        'Zoom','Adobe','Creative Cloud','Steam','Epic')
        foreach ($s in $startupEntries) {
            $heavy = $knownHeavy | Where-Object { $s.Name -match $_ -or $s.Command -match $_ }
            $tag   = if ($heavy) { '  [HIGH IMPACT]' } else { '' }
            Log-Info "  $($s.Name)$tag  -  $($s.Command)"
        }
    } else {
...
# Magic bytes / file extension mismatch audit (Deep tier only)

...
<#
.SYNOPSIS
    ShellKnight v1.001  -  Enterprise Endpoint Security & Remediation Tool
...
# ==============================================================================
# SHELLKNIGHT v1.001 CONFIGURATION
...
function Write-Log {
    ...
    if ($Script:LogReady) { $Script:LogWriter.WriteLine($line) }
        ...
        Invoke-SafeBlock -Label 'Remote access inventory' -Block {
            ...
            foreach ($tool in $remoteTools) {
                ...
                $svcs = @($Script:Cache_Services | Where-Object {
                    $svcPat = $tool.Services
                    $svcPat | Where-Object { $_.Name -like $_ -or $_.DisplayName -like $_ }
                })
                if ($svcs.Count -gt 0) {
                    $found     = $true
                ...
                $procs = @($Script:Cache_Processes | Where-Object {
                    $procPat = $tool.Procs
                    $procPat | Where-Object { $_.Name -like $_ }
                })
...
Log-Info $sepLine
Log-Info "  ShellKnight v1.001 - Report"
...
Write-Host "  ShellKnight v1.001 - Report" -ForegroundColor Cyan

...
<#
.SYNOPSIS
    ShellKnight v0.80  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.80  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($defThreats.Count -gt 0) {
        Log-Warn "Defender threat history  -  $($defThreats.Count) detection(s) in last 30 days:"
        foreach ($t in $defThreats | Sort-Object InitialDetectionTime -Descending | Select-Object -First 10) {
            ...
            Log-Warn "  $($t.InitialDetectionTime.ToString('yyyy-MM-dd'))  $name  -  $($t.Resources -join ', ')"
        }
    } else {

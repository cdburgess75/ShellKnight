...
<#
.SYNOPSIS
    ShellKnight v0.83  -  Enterprise Endpoint Security & Remediation Tool
...
.NOTES
    ...
    v0.83 - PowerShell script block audit: scans 4104 events for obfuscation.
            Startup enabled/disabled state from StartupApproved registry key.
            LAN Manager auth level auto-remediation: $SK_SetLMAuthLevel.
            Windows Firewall auto-enable: $SK_EnableFirewall.
            Meaningful INFO to screen: summary-level INFO prints, noise suppressed.
            Memory pressure report: current RAM usage vs total.
            Top CPU consumers: top 5 processes by CPU at scan time.
            Disk health: CIM pass/fail prediction query.
            Listening ports inventory: flags suspicious owners and RAT ports.
            Software version audit: flags outdated Adobe/Chrome/Edge/Java/7-Zip/VLC.
            Printer driver audit: print spooler config, PrintNightmare checks.
            License expiration: Office 365 and Windows activation status.
            Local admin audit: Administrator rename check, blank password check.
            Credential exposure: LSA protection, WDigest, Credential Guard.
            Scheduled task deep inspection: flags encoded PS commands in tasks.
            Version : v0.82 -> v0.83 per versioning rule.

    v0.82 - UltraVNC whitelist: when binary path contains CentraStage (Datto RMM),
            treated as legitimate Datto component, not RISKWARE-RAT. Logged as
            INFO under Datto RMM inventory instead of IOC.
            Additional RMM path whitelists added for other bundled VNC variants.
            Version : v0.81 -> v0.82 per versioning rule.

    v0.81 - Phase 21/15b ScreenConnect hardening: Event 7045 extracts exact binary
            path for targeted removal. Phase 15b AppData scan remains as backstop.
            Both set RogueScreenConnectRemoved flag to prevent double reporting.
            Defender threat history: grouped by date, ~Old Users and archive paths
            flagged separately as historical detections to reduce noise.
            LAN Manager auth level check added to hardening score deduction.
            Windows Firewall disabled deducts from security score.
            Version : v0.80 -> v0.81 per versioning rule.

    v0.80 - Remote access tool full inventory: ScreenConnect (all instances with ID),
            TeamViewer, AnyDesk, Splashtop, LogMeIn, BeyondTrust, Kaseya, NinjaRMM,
            ConnectWise Automate, Atera, Pulseway, Zoho Assist, Chrome Remote Desktop.
            [RISKWARE-RAT] classification: MeshAgent, Rustdesk, Action1, SimpleHelp,
            Ammyy Admin, NetSupport, UltraVNC/TightVNC. Counts as IOC, triggers
            Defender Quick Scan, appears in IOC section.
            Phase 15b: uses Event 7045 path directly for SC removal.
            VHD/VHDX/VMRS excluded from large file finder.
            Hyper-V host detection: role detected, VMs listed in machine info.
            Redirected folder scan: auto-detects non-C:\ drives with user home
            directories, scans for PUA executables in redirected Downloads/Desktop/
            AppData Temp. Default on. Reports username extracted from path.
            Version : v0.79 -> v0.80 per versioning rule.

    v0.79 - Phase progress indicator: [elapsed | Phase N/29] Phase name printed
            to screen at start of each phase. Works in interactive and Datto stdout.
            CIS Benchmark lite: Level 1 subset checks added to Phase 29.
            Startup impact classification: High/Medium/Low tiers with expanded
            known-impact database. Unknown updater/helper/launcher entries flagged.
            Version : v0.78 -> v0.79 per versioning rule.

    v0.78 - Windows Update Cache file count safety check: skips size enumeration
            if file count exceeds 50000 to prevent runaway runtimes.
            SM_* and HealthMailbox* accounts excluded from never-logged-in report.
            PST on network/cloud location detection: flags as WARN.
            Large OST file detection: flags OST files over configurable size (10 GB).
            CJIS compliance checks when law enforcement software detected.
            Defender threat history: Get-MpThreatDetection last 30 days, ~1 second.
            Defender Quick Scan on IOC hit: background job, doesn't block report.
            PowerShell script block audit: enables 4104 logging as hardening,
            scans existing events for obfuscation indicators.
            Startup impact report: enumerates startup entries with enabled/disabled status.
            Windows Update names: shows KB titles and severity via WUA COM object.
            File extension magic bytes audit: scoped to high-risk locations,
            executable-adjacent extensions only, capped at 5000 files/30 seconds.
            Version : v0.77 -> v0.78 per versioning rule.

    v0.77 - DYMOConnectLauncher and DYMO* added to LegitProcessNames.
            8.3 short filename profiles (containing ~) excluded from stale profile report.
            WER archive cleanup: folder delete/recreate instead of file-by-file iteration.
            N-able/N-central agent detection: flags as WARN (previous MSP remnant).
            Large file finder: configurable threshold (default 2 GB), Standard tier.
            Browser credential store check: Chrome/Edge/Firefox existence check.
            ScreenConnect Phase 15b: $Script:RogueScreenConnectRemoved flag suppresses
            duplicate Event 7045 IOC on same run after successful removal.
            Risk delta reporting: compares current grade to last run JSON, reports changes.
            HIPAA checks: screen lock timeout, Windows Firewall, consolidates existing
            findings into HIPAA Technical Safeguards section when healthcare software found.
            Acceptable use audit: gaming software and personal cloud storage detection.
            SMBv1 now deducts -20 from security score (same tier as no AV).
            NetBIOS disable: $SK_DisableNetBIOS (default off), sets TcpipNetbiosOptions=2.
            Version : v0.76 -> v0.77 per versioning rule.
...
$SK_DisableNetBIOS = $false         # Set $true to disable NetBIOS over TCP/IP on all adapters
$SK_SetLMAuthLevel = $false         # Set $true to auto-set LAN Manager auth level
$SK_LMAuthLevel    = 5              # Target LM auth level (5 = NTLMv2 only, refuse LM/NTLM)
                                     # WARNING: level 5 may break legacy devices
$SK_EnableFirewall = $false         # Set $true to auto-enable Windows Firewall on all profiles
$SK_VerboseScreen  = $false         # Set $true to show summary INFO messages on screen

# --- LARGE FILE FINDER ---
$SK_LargeFileThresholdGB = 2.0      # Flag files larger than this size (GB), 0 to disable
$SK_LargeOSTThresholdGB  = 10.0     # Flag Outlook OST files larger than this size (GB)
$SK_LargeFileScanPaths   = @(       # Paths to scan for large files
    "$env:SystemDrive\Users"
)

# --- MAGIC BYTES AUDIT ---
$SK_MagicBytesAudit      = $false   # Set $true to enable file extension magic bytes audit (Deep tier)
$SK_MagicBytesMaxFiles   = 5000     # Maximum files to scan before stopping
$SK_MagicBytesScanPaths  = @(       # Paths to scan for magic byte mismatches
...
$SK_RemoteAccessWarnUnknown  = $true    # WARN on tools not in Add/Remove Programs
...
Write-Host ("  ShellKnight v0.83  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
$Script:Config = @{
    ...
    MagicBytesScanPaths       = $SK_MagicBytesScanPaths
    # Remote access inventory
...
function Write-PhaseProgress {
    param([int]$PhaseNum, [int]$TotalPhases = 29, [string]$PhaseName)
...
function Log-Summary {
    param([string]$m)
    # Summary INFO — prints to screen when VerboseScreen enabled, always to log
    Write-Log -Message $m -Level INFO
    if ($Script:Config -and $Script:Config.VerboseScreen) {
        $ts = [datetime]::Now.ToString('yyyy-MM-dd HH:mm:ss')
        Write-Host "$ts  [INFO]     $m" -ForegroundColor White
    }
}
...
# PHASE 0: HARDWARE & OS DETECTION
# ==================================================================================================
Write-PhaseProgress -PhaseNum 0 -PhaseName 'Hardware & OS Detection'
...
# PHASE 1: DYNAMIC INTELLIGENCE DOWNLOAD
# ==================================================================================================
Write-PhaseProgress -PhaseNum 1 -PhaseName 'Dynamic Intelligence Download'
...
# PHASE 2: MACHINE INFORMATION BLOCK
# ==================================================================================================
Write-PhaseProgress -PhaseNum 2 -PhaseName 'Machine Information'
...
try {
    ...
    try {
        ...
        if ($hvFeature -and $hvFeature.State -eq 'Enabled') {
            ...
            Log-Info "Hyper-V host detected  -  $($vms.Count) VM(s): $vmList"
        } else {
            $Script:HWInfo.IsHyperVHost = $false
        }
    } catch { $Script:HWInfo.IsHyperVHost = $false }
...
# PHASE 3: PROCESS TERMINATION
# ==================================================================================================
Write-PhaseProgress -PhaseNum 3 -PhaseName 'Process Termination'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 4 -PhaseName 'Filesystem Cleanup'
...
# PHASE 5: BROWSER EXTENSION ARTIFACT REMOVAL
# ==================================================================================================
Write-PhaseProgress -PhaseNum 5 -PhaseName 'Browser Extension Removal'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 6 -PhaseName 'Registry Uninstall'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 7 -PhaseName 'Service Removal'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 8 -PhaseName 'Scheduled Task Removal'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 9 -PhaseName 'Run Key Cleanup'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 10 -PhaseName 'Startup Folder Cleanup'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 11 -PhaseName 'Browser Policy Cleanup'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 12 -PhaseName 'Defender Exclusion Cleanup'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 13 -PhaseName 'Hosts File Inspection'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 14 -PhaseName 'WMI Persistence Audit'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 15 -PhaseName 'Trojan/Malware IOC Detection'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 16 -PhaseName 'RiskWare/Exploit/ScreenConnect'
...
try {
    ...
    foreach ($profile in $userProfiles) {
        ...
        foreach ($scExe in $scExes) {
            ...
            # Check if THIS specific instance is in Add/Remove Programs
            # Match by install location path containing the same AppData folder, not just any SC entry
            $inARP = $false
            try {
                ...
                foreach ($arp in $arpEntries) {
                    $arpLoc = $arp.InstallLocation
                    if ($arpLoc -and $scFolder -like "*$arpLoc*") { $inARP = $true; break }
                    # Also check if instance ID matches an ARP entry's uninstall string
                    if ($instanceID -and $arp.UninstallString -match [regex]::Escape($instanceID)) {
                        $inARP = $true; break
                    }
                }
            } catch { }
...
# PHASE 16: REBOOT REQUIREMENT CHECK
# ==================================================================================================
Write-PhaseProgress -PhaseNum 17 -PhaseName 'Reboot Check'
...
# PHASE 17: MALWAREBAZAAR HASH LOOKUP + NEO23x0 FALLBACK + DEFENDER FALLBACK
# ==================================================================================================
Write-PhaseProgress -PhaseNum 18 -PhaseName 'MalwareBazaar Hash Lookup'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 19 -PhaseName 'Disk Space Cleanup'
...
try {
    ...
    # Safety check - skip size enumeration if file count is enormous (prevents runaway runtimes)
    $wuCachePath = 'C:\Windows\SoftwareDistribution\Download'
    $wuFileCount = @(Get-ChildItem -LiteralPath $wuCachePath -Recurse -Force -ErrorAction SilentlyContinue).Count
    if ($wuFileCount -gt 50000) {
        Log-Info "Windows Update Cache  -  $wuFileCount files detected, using fast delete/recreate"
        $wuSize = (Get-FolderSizeBytes $wuCachePath)
        try {
            Remove-Item -LiteralPath $wuCachePath -Recurse -Force -ErrorAction Stop
            New-Item -Path $wuCachePath -ItemType Directory -Force | Out-Null
            $freedMB = [math]::Round($wuSize / 1MB, 1)
            Log-Success "Cleaned Windows Update Cache  -  $wuFileCount files / $freedMB MB | Freed: $freedMB MB"
            $Script:SpaceFreed += $wuSize
            $Script:Counters.ActionsTaken++
        } catch { Log-Warn "Windows Update Cache fast delete failed  -  $($_.Exception.Message)" }
    } else {
        Remove-FolderContents -Path $wuCachePath -Label 'Windows Update Cache'
    }
    # Restart services
...
# WER cleanup - fast folder delete/recreate instead of file-by-file iteration
foreach ($werPath in @(
    'C:\ProgramData\Microsoft\Windows\WER\ReportArchive',
    'C:\ProgramData\Microsoft\Windows\WER\ReportQueue'
)) {
    $werLabel = if ($werPath -match 'Archive') { 'WER Report Archive' } else { 'WER Report Queue' }
    if (Test-Path -LiteralPath $werPath) {
        $beforeCount = @(Get-ChildItem -LiteralPath $werPath -Force -ErrorAction SilentlyContinue).Count
        $beforeBytes = (Get-FolderSizeBytes $werPath)
        try {
            Remove-Item -LiteralPath $werPath -Recurse -Force -ErrorAction Stop
            New-Item -Path $werPath -ItemType Directory -Force | Out-Null
            $freedMB = [math]::Round($beforeBytes / 1MB, 1)
            Log-Success "Cleaned $werLabel  -  Before: $beforeCount files / $freedMB MB | After: 0 files | Freed: $freedMB MB"
            $Script:SpaceFreed += $beforeBytes
            $Script:Counters.ActionsTaken++
            $Script:Counters.FilesRemoved += $beforeCount
        } catch {
            Remove-FolderContents -Path $werPath -Label $werLabel
        }
    }
}
...
# PHASE 19: RECENTLY INSTALLED SOFTWARE REPORT
# ==================================================================================================
Write-PhaseProgress -PhaseNum 20 -PhaseName 'Recent Software Report'
...
# PHASE 20: TEMP FILE AGE REPORT
# ==================================================================================================
Write-PhaseProgress -PhaseNum 21 -PhaseName 'Temp File Age Report'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 22 -PhaseName 'Event Log IOC Check'
...
try {
    ...
    if ($svcEvents.Count -gt 0) {
        ...
        foreach ($key in $svcGroups.Keys) {
            ...
            # Suppress if ScreenConnect rogue was already removed in Phase 15b
            if ($Script:RogueScreenConnectRemoved -and $g['SvcPath'] -match 'screenconnect') {
                Log-Info "Event 7045 ScreenConnect  -  suppressed (rogue instance removed in Phase 15b)"
                continue
            }

            # Phase 21 targeted SC removal using exact path from event log
            if ($g['SvcPath'] -match 'screenconnect' -and -not $Script:RogueScreenConnectRemoved) {
                # Extract instance ID from path or service name
                $eventInstanceID = $null
                if ($g['SvcName'] -match '\(([a-f0-9\-]{8,})\)') { $eventInstanceID = $Matches[1] }
                elseif ($g['SvcPath'] -match '&s=([a-f0-9\-]{8,})') { $eventInstanceID = $Matches[1] }

                $isManagedSC = $eventInstanceID -and $Script:Config.SCInstanceID -and
                               ($eventInstanceID -eq $Script:Config.SCInstanceID)

                if (-not $isManagedSC) {
                    Log-IOC "Event 7045 ScreenConnect  -  non-managed instance detected: $($eventInstanceID ?? 'unknown')"
                    Log-IOC "  Service: $($g['SvcName'])  |  Path: $($g['SvcPath'])"

                    if ($Script:Config.SCRemoveRogue) {
                        # Stop and remove service using exact name from event
                        try {
                            Stop-Service -Name $g['SvcName'] -Force -ErrorAction SilentlyContinue
                            Start-Sleep -Milliseconds 500
                            & sc.exe delete $g['SvcName'] 2>$null | Out-Null
                            Log-Success "Removed rogue SC service (via Event 7045): $($g['SvcName'])"
                        } catch { }

                        # Extract folder path from binary path and delete it
                        try {
                            ...
                            if ($exeFolder -and (Test-Path -LiteralPath $exeFolder)) {
                                ...
                                Log-Success "Removed rogue SC folder (via Event 7045): $exeFolder"
                                $Script:RogueScreenConnectRemoved = $true
                            }
                        } catch { Log-Fail "Could not remove SC folder from event path  -  $($_.Exception.Message)" }
                    }
                    continue
                }
            }
...
try {
    ...
    foreach ($u in $localUsers) {
        ...
        # Skip Exchange health and service accounts
        if ($u.Name -match '^(SM_|HealthMailbox)') { continue }
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 23 -PhaseName 'Hardening Checks'
...
try {
    ...
    if ($llmnr -ne 0) {
        if ($Script:Config.DisableLLMNR) {
            ...
            } catch { Log-Fail "LLMNR disable failed  -  $($_.Exception.Message)" }
        } else { Log-Warn "LLMNR may be enabled  -  recommend disabling via GPO" }
    } else { Log-Summary "LLMNR  -  disabled via policy (OK)" }
} catch { Log-Info "LLMNR check skipped" }

# LAN Manager auth level auto-remediation
try {
    ...
    if ($null -eq $lmCurrent -or $lmCurrent -lt 3) {
        if ($Script:Config.SetLMAuthLevel) {
            ...
            Log-Harden "LAN Manager auth level set to $($Script:Config.LMAuthLevel) (NTLMv2 only)  -  NTLMv1/LM authentication disabled"
        } else {
            Log-Warn "LAN Manager auth level is $lmCurrent  -  recommend setting to 5 (NTLMv2 only)"
        }
    } else { Log-Summary "LAN Manager auth level: $lmCurrent (OK)" }
} catch { Log-Info "LAN Manager auth level check skipped  -  $($_.Exception.Message)" }

# Windows Firewall auto-enable
try {
    $fwProfiles = @(Get-NetFirewallProfile -ErrorAction Stop)
    $fwDisabled = @($fwProfiles | Where-Object { $_.Enabled -eq $false })
    if ($fwDisabled.Count -gt 0) {
        if ($Script:Config.EnableFirewall) {
            foreach ($profile in $fwDisabled) {
            ...
            Log-Warn "Windows Firewall disabled on: $($fwDisabled.Profile -join ', ')"
        }
    } else { Log-Summary "Windows Firewall  -  enabled on all profiles (OK)" }
} catch { Log-Info "Windows Firewall check skipped  -  $($_.Exception.Message)" }
...
try {
    ...
    if ($netbiosEnabled.Count -gt 0) {
        if ($Script:Config.DisableNetBIOS) {
            $disabled = 0
            foreach ($adapter in $netbiosEnabled) {
                try {
                    $result = $adapter.SetTcpipNetbios(2)
                    if ($result.ReturnValue -eq 0) { $disabled++ }
                } catch { }
            }
            if ($disabled -gt 0) {
                Log-Harden "NetBIOS disabled on $disabled adapter(s)  -  MITM attack vector eliminated"
            } else {
                Log-Warn "NetBIOS over TCP/IP may be enabled on $($netbiosEnabled.Count) adapter(s)  -  could not disable automatically"
            }
        } else {
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 24 -PhaseName 'USB/Removable Media Audit'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 25 -PhaseName 'Network Connection Audit'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 26 -PhaseName 'Ransomware Canary'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 27 -PhaseName 'Windows Update Check'
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 28 -PhaseName 'Stale Profile Report'
...
try {
    ...
    $profiles = @(Get-ChildItem 'C:\Users' -Directory -ErrorAction SilentlyContinue |
                  Where-Object {
                      ...
                      $_.Name -notmatch '~' -and      # 8.3 short filename artifacts
...
# ==================================================================================================
Write-PhaseProgress -PhaseNum 29 -PhaseName 'Trend Tracking'
...
# ==================================================================================================
# PHASE 29: EXTENDED CHECKS (v0.77)  -  Large Files, Browser Creds, N-able, HIPAA, Acceptable Use
# ==================================================================================================
Write-PhaseProgress -PhaseNum 29 -PhaseName 'Extended Checks'
Log-Info '--- Phase 29: Extended Checks ---'

# Large file finder
if ($Script:Config.LargeFileThresholdGB -gt 0) {
    try {
        $thresholdBytes = [long]($Script:Config.LargeFileThresholdGB * 1GB)
        $largeFileExclExt = @('.vhd','.vhdx','.vmrs','.vmdk','.vdi','.ova','.ovf')
        $largeFiles = @()
        foreach ($scanPath in $Script:Config.LargeFileScanPaths) {
            if (-not (Test-Path -LiteralPath $scanPath)) { continue }
            $found = @(Get-ChildItem -LiteralPath $scanPath -Recurse -Force -File `
                       -ErrorAction SilentlyContinue |
                       Where-Object {
                           $_.Length -gt $thresholdBytes -and
                           $largeFileExclExt -notcontains $_.Extension.ToLower()
                       } |
                       Select-Object -First 20)
            $largeFiles += $found
        }
        if ($largeFiles.Count -gt 0) {
            ...
            foreach ($f in $largeFiles | Sort-Object Length -Descending) {
                ...
                Log-Warn "  $sizeGB GB  -  $($f.FullName)"
            }
        } else {
            Log-Info "Large file finder  -  no files over $($Script:Config.LargeFileThresholdGB) GB found"
        }
    } catch { Log-Info "Large file finder skipped  -  $($_.Exception.Message)" }
}

# Browser credential store check (existence only - no content read)
try {
    $credStores = @(
    ...
    foreach ($store in $credStores) {
        if (Test-Path -LiteralPath $store.Path) { $foundStores += $store.Browser }
    }
    if ($foundStores.Count -gt 0) {
        Log-Info "Browser credential stores present: $($foundStores -join ', ')  -  ensure disk encryption is enabled"
        if (-not $bitlockerWarn -eq $false) {
            Log-Warn "Browser credentials exist but BitLocker is OFF  -  credentials at risk if device is lost"
        }
    } else {
        Log-Info "Browser credential stores  -  none found"
    }
} catch { Log-Info "Browser credential check skipped  -  $($_.Exception.Message)" }

# N-able / N-central agent detection (previous MSP remnant)
try {
    $nableServices = @('NableAgent','N-able Agent','RpcAgentSvc','Windows Agent Service')
    $nableFound = @(Get-Service -ErrorAction SilentlyContinue |
                    Where-Object { $nableServices -contains $_.Name -or
                                   $_.DisplayName -match 'N-able|N-central|Nable' })
    $nablePaths = @(
        'C:\Program Files\N-able Technologies',
        'C:\Program Files (x86)\N-able Technologies',
        'C:\Program Files\N-able Technologies\AVDefender'
    )
    $nableInstalled = @($nablePaths | Where-Object { Test-Path -LiteralPath $_ })
    if ($nableFound.Count -gt 0 -or $nableInstalled.Count -gt 0) {
        Log-Warn "N-able/N-central agent detected  -  possible previous MSP remnant, verify with client"
        if ($nableInstalled.Count -gt 0) {
            Log-Warn "  N-able software path found: $($nableInstalled -join ', ')"
        }
        if ($nableFound.Count -gt 0) {
            Log-Warn "  N-able services: $($nableFound.DisplayName -join ', ')"
        }
    } else {
        Log-Info "N-able/N-central  -  not detected"
    }
} catch { Log-Info "N-able check skipped  -  $($_.Exception.Message)" }

# HIPAA Technical Safeguards check
try {
    # Detect healthcare software
    $healthcareSoftware = @('Allscripts','MiSys','Epic','eClinicalWorks','Meditech','Cerner',
                            'NextGen','AdvancedMD','Kareo','DrChrono','AthenaHealth',
                            'QuadraMed','McKesson','Greenway','Netsmart')
    $installedSoftware = @(Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*',
                            'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' `
                           -ErrorAction SilentlyContinue | Select-Object -ExpandProperty DisplayName)
    $hipaaRelevant = $installedSoftware | Where-Object { $n = $_; $healthcareSoftware | Where-Object { $n -match $_ } }

    if ($hipaaRelevant) {
        Log-Info "HIPAA-relevant software detected: $($hipaaRelevant -join ', ')"
        Log-Info "--- HIPAA Technical Safeguards Assessment ---"

        # Screen lock / auto-logoff
        $scrTimeout = (Get-ItemProperty 'HKCU:\Control Panel\Desktop' -Name 'ScreenSaveTimeOut' `
                       -ErrorAction SilentlyContinue).ScreenSaveTimeOut
        $scrActive  = (Get-ItemProperty 'HKCU:\Control Panel\Desktop' -Name 'ScreenSaverIsSecure' `
                       -ErrorAction SilentlyContinue).ScreenSaverIsSecure
        if ($scrTimeout -and [int]$scrTimeout -le 900 -and $scrActive -eq '1') {
            Log-Info "  [HIPAA] Auto-logoff/screen lock  -  enabled (OK)"
        } else {
            Log-Warn "  [HIPAA] Auto-logoff/screen lock  -  not configured or over 15 minutes"
        }

        # Encryption
        if ($bitlockerWarn) {
            Log-Warn "  [HIPAA] Encryption at rest  -  BitLocker OFF (HIPAA requires encryption)"
        } else {
            Log-Info "  [HIPAA] Encryption at rest  -  BitLocker ON (OK)"
        }

        # Audit logging
        Log-Info "  [HIPAA] Audit logging  -  process creation auditing enabled via Phase 22 (OK)"

        # Access controls
        if ($inactiveAccounts.Count -gt 0) {
            Log-Warn "  [HIPAA] Access control  -  $($inactiveAccounts.Count) inactive account(s) present (review)"
        } else {
            Log-Info "  [HIPAA] Access control  -  no inactive accounts detected (OK)"
        }

        # Windows Firewall
        $fwProfiles = @(Get-NetFirewallProfile -ErrorAction SilentlyContinue)
        $fwAllOn = ($fwProfiles | Where-Object { $_.Enabled -eq $false }).Count -eq 0
        if ($fwAllOn) {
            Log-Info "  [HIPAA] Network protection  -  Windows Firewall enabled on all profiles (OK)"
        } else {
            Log-Warn "  [HIPAA] Network protection  -  Windows Firewall disabled on one or more profiles"
        }
    } else {
        Log-Info "HIPAA check  -  no healthcare software detected, skipping HIPAA assessment"
    }
} catch { Log-Info "HIPAA check skipped  -  $($_.Exception.Message)" }

# Acceptable use audit
try {
    $gamingSoftware = @('Steam','EpicGamesLauncher','Battle.net','GOGGalaxy','Origin',
                        'EADesktop','Ubisoft Connect','Xbox','GameBar')
    $personalCloud  = @('Dropbox','GoogleDriveFS','OneDrive Personal','iCloudDrive',
                        'pCloud','MEGA','Sync')

    $foundGaming = @(Get-Process -ErrorAction SilentlyContinue |
                     Where-Object { $gamingSoftware -contains $_.Name })
    if ($foundGaming.Count -gt 0) {
        Log-Warn "Acceptable use  -  gaming software running: $($foundGaming.Name -join ', ')  -  verify with client"
    }

    $foundCloud = @(Get-Process -ErrorAction SilentlyContinue |
                    Where-Object { $n = $_.Name; $personalCloud | Where-Object { $n -match $_ } })
    if ($foundCloud.Count -gt 0) {
        Log-Info "Personal cloud storage running: $($foundCloud.Name -join ', ')  -  verify authorized for business use"
    }

    if ($foundGaming.Count -eq 0 -and $foundCloud.Count -eq 0) {
        Log-Info "Acceptable use  -  no gaming or unauthorized cloud storage detected"
    }
} catch { Log-Info "Acceptable use check skipped  -  $($_.Exception.Message)" }
...
try {
    ...
    if ($usedPct -gt 85) {
        Log-Warn "Memory pressure: $usedPct% used ($usedMB MB of $totalMB MB)  -  consider upgrade or investigation"
    } else {
        Log-Summary "Memory: $usedPct% used ($usedMB MB of $totalMB MB)  -  OK"
    }
} catch { Log-Info "Memory pressure check skipped  -  $($_.Exception.Message)" }
...
try {
    ...
    if ($topProcs.Count -gt 0) {
        Log-Summary "Top CPU consumers at scan time:"
        foreach ($p in $topProcs) {
            $cpuSec = [math]::Round($p.CPU, 1)
            Log-Info "  $($p.Name) (PID $($p.Id))  -  $cpuSec CPU seconds"
        }
    }
} catch { Log-Info "CPU consumers check skipped  -  $($_.Exception.Message)" }

# Disk health - CIM pass/fail prediction
try {
    $diskStatus = @(Get-CimInstance -Namespace root\wmi -ClassName MSStorageDriver_FailurePredictStatus `
                    -ErrorAction Stop)
    foreach ($disk in $diskStatus) {
        if ($disk.PredictFailure) {
            Log-Warn "Disk health WARNING: drive failure predicted  -  immediate backup and replacement recommended"
            $Script:Counters.IOCsFound++
        } else {
            Log-Summary "Disk health: no failure predicted (OK)"
        }
    }
} catch { Log-Info "Disk health check skipped (virtual or unsupported hardware)" }

# Listening ports inventory
try {
    $suspectOwners = @('powershell','cmd','wscript','cscript','mshta','regsvr32','rundll32','nc','ncat','netcat')
    $ratPorts      = @(4444,4445,1234,31337,8888,9999,6666,1337,50050,60000)
    ...
    foreach ($l in $listeners) {
        ...
        $isSuspectOwner = $suspectOwners | Where-Object { $procName -match $_ }

        if ($isRatPort -or $isSuspectOwner) {
            $reason = if ($isRatPort) { 'known RAT/C2 port' } else { 'suspicious process owner' }
            Log-IOC "Suspicious listener: $($l.LocalAddress):$($l.LocalPort)  -  $procName  -  $reason"
            $suspectFound += "$($l.LocalPort)/$procName"
            $Script:Counters.IOCsFound++
        }
    }
    if ($suspectFound.Count -eq 0) {
        Log-Summary "Listening ports: $($listeners.Count) ports checked  -  no suspicious listeners"
    }
} catch { Log-Info "Listening ports check skipped  -  $($_.Exception.Message)" }

# Software version audit
try {
    $softwareVersions = @{
        'Google Chrome'   = @{ MinVersion = '124.0'; Current = '148.0' }
        'Microsoft Edge'  = @{ MinVersion = '124.0'; Current = '148.0' }
        'Adobe Acrobat'   = @{ MinVersion = '24.0';  Current = '26.0'  }
        'Adobe Reader'    = @{ MinVersion = '24.0';  Current = '26.0'  }
        '7-Zip'           = @{ MinVersion = '23.0';  Current = '24.0'  }
        'VLC'             = @{ MinVersion = '3.0.20';Current = '3.0.21'}
        'Java'            = @{ MinVersion = '21.0';  Current = '21.0'  }
        'Zoom'            = @{ MinVersion = '6.0';   Current = '6.4'   }
        'Foxit PDF Reader'= @{ MinVersion = '2024.0';Current = '2026.0'}
    }
    ...
    foreach ($swName in $softwareVersions.Keys) {
        $match = @($installed | Where-Object { $_.DisplayName -match [regex]::Escape($swName) })
        if ($match.Count -gt 0 -and $match[0].DisplayVersion) {
            $ver = $match[0].DisplayVersion
            $minVer = $softwareVersions[$swName].MinVersion
            if ([version]::TryParse($ver.Split(' ')[0], [ref]$null) -and
                [version]::TryParse($minVer, [ref]$null)) {
                if ([version]$ver.Split(' ')[0] -lt [version]$minVer) {
                    Log-Warn "Outdated software: $swName v$ver  -  recommend updating (current: $($softwareVersions[$swName].Current))"
                    $outdatedFound += $swName
                }
            }
        }
    }
    if ($outdatedFound.Count -eq 0) {
        Log-Summary "Software version audit  -  all checked software is current"
    }
} catch { Log-Info "Software version audit skipped  -  $($_.Exception.Message)" }

# Printer driver audit - PrintNightmare checks
try {
    $spooler = Get-Service -Name Spooler -ErrorAction SilentlyContinue
    if ($spooler -and $spooler.Status -eq 'Running') {
        # Check if Point and Print restrictions are configured
        $pnpRestrict = (Get-ItemProperty 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Printers\PointAndPrint' `
                        -Name 'RestrictDriverInstallationToAdministrators' -ErrorAction SilentlyContinue).RestrictDriverInstallationToAdministrators
        if ($pnpRestrict -ne 1) {
            Log-Warn "PrintNightmare exposure: Point and Print not restricted to admins  -  recommend setting RestrictDriverInstallationToAdministrators=1"
        } else {
            Log-Summary "Print spooler: Point and Print restricted to admins (OK)"
        }
        # List third-party drivers
        $thirdPartyDrivers = @(Get-PrinterDriver -ErrorAction SilentlyContinue |
                               Where-Object { $_.Manufacturer -notmatch 'Microsoft' })
        if ($thirdPartyDrivers.Count -gt 0) {
            Log-Info "Third-party printer drivers ($($thirdPartyDrivers.Count)): $($thirdPartyDrivers.Name -join ', ')"
        }
    } else {
        Log-Summary "Print spooler: not running (OK for servers)"
    }
} catch { Log-Info "Printer driver audit skipped  -  $($_.Exception.Message)" }

# License expiration check
try {
    # Windows activation
    $winLicense = Get-CimInstance SoftwareLicensingProduct -ErrorAction SilentlyContinue |
    ...
    if ($winLicense) {
        ...
        if ($winLicense.LicenseStatus -ne 1) {
            Log-Warn "Windows activation: $licStatus  -  verify licensing"
        } else {
            Log-Summary "Windows activation: Licensed (OK)"
        }
    }
    # Office activation check
    $officePath = @('HKLM:\SOFTWARE\Microsoft\Office\16.0\Common\Licensing',
                    'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\16.0\Common\Licensing')
    $officeKey = $officePath | Where-Object { Test-Path $_ } | Select-Object -First 1
    if ($officeKey) {
        Log-Summary "Office 16: licensing registry key present"
    }
} catch { Log-Info "License check skipped  -  $($_.Exception.Message)" }

# Local admin account audit
try {
    # Check if built-in Administrator account has been renamed
    $builtInAdmin = Get-LocalUser -ErrorAction SilentlyContinue |
                    Where-Object { $_.SID -match '-500$' }
    if ($builtInAdmin) {
        if ($builtInAdmin.Name -eq 'Administrator') {
            Log-Warn "Local admin audit: built-in Administrator account not renamed  -  CIS recommendation: rename it"
        } else {
            Log-Summary "Local admin audit: built-in Administrator renamed to '$($builtInAdmin.Name)' (OK)"
        }
    }
    # Check for blank password local admins
    $adminGroup = Get-LocalGroupMember -Group 'Administrators' -ErrorAction SilentlyContinue
    foreach ($member in $adminGroup | Where-Object { $_.ObjectClass -eq 'User' -and $_.PrincipalSource -eq 'Local' }) {
        try {
            $user = Get-LocalUser -Name $member.Name.Split('\')[-1] -ErrorAction SilentlyContinue
            if ($user -and $user.PasswordRequired -eq $false) {
                Log-Warn "Local admin '$($member.Name)' has no password required  -  critical security gap"
            }
        } catch { }
    }
} catch { Log-Info "Local admin audit skipped  -  $($_.Exception.Message)" }

# Credential exposure check
try {
    # WDigest plaintext passwords in memory
    ...
    if ($wdigest -eq 1) {
        Log-IOC "WDigest enabled: plaintext credentials stored in memory (UseLogonCredential=1)  -  attackers can dump passwords"
        $Script:Counters.IOCsFound++
    } else {
        Log-Summary "WDigest: plaintext credential caching disabled (OK)"
    }
    ...
    if ($lsaProtect -ne 1) {
        Log-Warn "LSA protection (RunAsPPL) not enabled  -  recommend enabling to protect credentials"
    } else {
        Log-Summary "LSA protection: enabled (OK)"
    }
    ...
    if ($credGuard -eq 1) {
        Log-Summary "Credential Guard: virtualization-based security enabled (OK)"
    } else {
        Log-Info "Credential Guard: not enabled  -  consider enabling on modern hardware"
    }
} catch { Log-Info "Credential exposure check skipped  -  $($_.Exception.Message)" }

# Scheduled task deep inspection - flag encoded/obfuscated commands
try {
    $suspectTasks = @()
    ...
    foreach ($task in $allTasks) {
        foreach ($action in $task.Actions) {
            ...
            if ($obfuscPattern.IsMatch($cmdLine)) {
                ...
                if (-not $isLegit) {
                    ...
                    Log-IOC "  Path: $($task.TaskPath)  |  Command: $($cmdLine.Substring(0,[Math]::Min(120,$cmdLine.Length)))"
                    $suspectTasks += $task.TaskName
                    $Script:Counters.IOCsFound++
                }
            }
        }
    ...
    if ($suspectTasks.Count -eq 0) {
        Log-Summary "Scheduled task inspection: $($allTasks.Count) active tasks checked  -  no obfuscated commands found"
    }
} catch { Log-Info "Scheduled task deep inspection skipped  -  $($_.Exception.Message)" }
...
try {
    ...
    if ($sbEnabled -eq 1) {
        $obfuscKeywords = @('EncodedCommand','FromBase64String','IEX','Invoke-Expression',
                            ...
                            'reflection.assembly','[convert]::','[system.text')
        ...
        if ($obfuscated.Count -gt 0) {
            ...
            Log-Summary "PowerShell script block audit: $($sbEvents.Count) events checked  -  no obfuscation found"
        }
    } else {
        Log-Info "PowerShell script block logging not yet enabled on this machine  -  enable via Phase 29 hardening"
    }
} catch { Log-Info "PowerShell script block audit skipped  -  $($_.Exception.Message)" }
try {
    $ostThreshold = [long]($Script:Config.LargeOSTThresholdGB * 1GB)
    $userDirs = @(Get-ChildItem 'C:\Users' -Directory -ErrorAction SilentlyContinue |
                  Where-Object { $_.Name -notmatch '^(Public|Default|All Users)$' })
    foreach ($ud in $userDirs) {
        # Large OST files
        $osts = @(Get-ChildItem -LiteralPath $ud.FullName -Recurse -Filter '*.ost' `
                  -Force -ErrorAction SilentlyContinue |
                  Where-Object { $_.Length -gt $ostThreshold })
        foreach ($ost in $osts) {
            $sizeGB = [math]::Round($ost.Length / 1GB, 2)
            Log-Warn "Large Outlook OST: $sizeGB GB  -  $($ost.FullName)  -  consider archiving old email"
        }
        # PST on network or cloud paths
        $psts = @(Get-ChildItem -LiteralPath $ud.FullName -Recurse -Filter '*.pst' `
                  -Force -ErrorAction SilentlyContinue |
                  Where-Object { $_.FullName -match 'OneDrive|SharePoint|Dropbox|Box|Google Drive|network' })
        foreach ($pst in $psts) {
            $sizeGB = [math]::Round($pst.Length / 1GB, 2)
            Log-Warn "PST on cloud/network location (unsupported by Microsoft): $sizeGB GB  -  $($pst.FullName)"
        }
    }
} catch { Log-Info "PST/OST check skipped  -  $($_.Exception.Message)" }

# Defender threat history - last 30 days
try {
    ...
    if ($defThreats.Count -gt 0) {
        $activeThreats = @()
        $historicalThreats = @()
        $archivePatterns = @('~Old Users','~old users','archive','backup','quarantine','AdwCleaner')
        ...
        foreach ($t in $defThreats) {
            ...
            if ($isHistorical) { $historicalThreats += $entry } else { $activeThreats += $entry }
        }
        ...
        if ($historicalThreats.Count -gt 0) {
            Log-Info "Defender threat history  -  $($historicalThreats.Count) historical detection(s) in archive/old user paths (lower priority):"
        ...
        if ($activeThreats.Count -eq 0 -and $historicalThreats.Count -gt 0) {
            Log-Info "Defender threat history  -  all detections in archive/old user paths, no active path threats"
        }
    } else {
        Log-Info "Defender threat history  -  no detections in last 30 days"
    }
} catch { Log-Info "Defender threat history skipped  -  $($_.Exception.Message)" }

# Defender Quick Scan on IOC hit - runs as background job
if ($Script:Counters.IOCsFound -gt 0) {
    try {
        $mpStatus = Get-MpComputerStatus -ErrorAction Stop
        if ($mpStatus.AMServiceEnabled -and $mpStatus.RealTimeProtectionEnabled) {
            Start-MpScan -ScanType QuickScan -AsJob -ErrorAction Stop | Out-Null
            Log-Info "Defender Quick Scan initiated in background  -  IOCs detected, scan running asynchronously"
        }
    } catch { Log-Info "Defender Quick Scan could not be initiated  -  $($_.Exception.Message)" }
}

# PowerShell script block logging - enable and audit
try {
    $sbPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging'
    $sbEnabled = (Get-ItemProperty $sbPath -Name 'EnableScriptBlockLogging' -ErrorAction SilentlyContinue).EnableScriptBlockLogging
    if ($sbEnabled -ne 1) {
        ...
        Log-Harden "PowerShell script block logging (4104) enabled  -  obfuscation audit available on next run"
    } else {
        # Audit existing 4104 events for obfuscation indicators
        $obfuscationPatterns = @(
            'EncodedCommand','FromBase64String','IEX','Invoke-Expression',
            'DownloadString','WebClient','bypass','hidden','noprofile',
            'reflection.assembly','[convert]::','[system.text'
        )
        $sbEvents = @(Get-WinEvent -FilterHashtable @{
        ...
        $obfuscated = @($sbEvents | Where-Object {
            $msg = $_.Message.ToLower()
            $obfuscationPatterns | Where-Object { $msg -match $_ }
        })
        if ($obfuscated.Count -gt 0) {
            Log-IOC "PowerShell obfuscation indicators found  -  $($obfuscated.Count) suspicious script block(s):"
            foreach ($e in $obfuscated | Select-Object -First 5) {
                Log-IOC "  $($e.TimeCreated.ToString('yyyy-MM-dd HH:mm:ss'))  $($e.Message.Substring(0,[Math]::Min(120,$e.Message.Length)))"
            }
            $Script:Counters.IOCsFound += $obfuscated.Count
        } else {
            Log-Info "PowerShell script block audit  -  no obfuscation indicators found in last 7 days"
        }
    }
} catch { Log-Info "PowerShell script block audit skipped  -  $($_.Exception.Message)" }

# Startup impact report - with enabled/disabled state
try {
    $startupEntries = @(Get-CimInstance Win32_StartupCommand -ErrorAction SilentlyContinue)

    # Build enabled/disabled lookup from StartupApproved registry
    $startupApproved = @{}
    $approvedPaths = @(
    ...
    foreach ($approvedPath in $approvedPaths) {
        if (Test-Path $approvedPath) {
            $props = Get-ItemProperty $approvedPath -ErrorAction SilentlyContinue
            if ($props) {
                $props.PSObject.Properties | Where-Object { $_.Name -notmatch '^PS' } | ForEach-Object {
                    # First byte: 2 = enabled, 3 = disabled
                    $firstByte = if ($_.Value -is [byte[]]) { $_.Value[0] } else { 2 }
                    $startupApproved[$_.Name] = ($firstByte -eq 2)
                }
            }
        }
    }
    ...
    if ($startupEntries.Count -gt 0) {
        Log-Summary "Startup entries  -  $($startupEntries.Count) found:"
        $highImpact   = @('OneDrive','Dropbox','GoogleDrive','Box','Steam','EpicGames','Battle.net','Discord','Spotify','Adobe','Creative Cloud','Teams','Slack','Skype','iTunes','iCloud','Cortana','Xbox','GameBar','Nvidia','GeForce')
        $mediumImpact = @('Chrome','Edge','Zoom','QuickBooks','Acrobat','Reader','Java','Malwarebytes','Avast','AVG','McAfee','Sophos','ESET')
        $suspectPatt  = @('updater','helper','launcher','scheduler','agent','notif','tray','sync')

        foreach ($s in $startupEntries) {
            $tier = 'LOW'; $tag = ''
            if ($highImpact   | Where-Object { $s.Name -match $_ -or $s.Command -match $_ }) { $tier = 'HIGH'; $tag = '  [HIGH IMPACT]' }
            elseif ($mediumImpact | Where-Object { $s.Name -match $_ -or $s.Command -match $_ }) { $tier = 'MED'; $tag = '  [MED IMPACT]' }
            elseif ($suspectPatt  | Where-Object { $s.Name -match $_ -or $s.Command -match $_ }) { $tag = '  [REVIEW]' }

            # Check enabled/disabled state
            $isEnabled = $startupApproved.ContainsKey($s.Name) ? $startupApproved[$s.Name] : $true
            $stateTag  = if ($isEnabled) { '[ENABLED] ' } else { '[DISABLED]' }

            Log-Info "  [$tier] $stateTag $($s.Name)$tag  -  $($s.Command)"
        }
    } else {
        Log-Summary "Startup entries  -  none found"
    }
} catch { Log-Info "Startup report skipped  -  $($_.Exception.Message)" }

# CJIS compliance checks - law enforcement software detection
try {
    $cjisSoftware = @('Tyler Technologies','New World','Motorola Solutions','Axon','PremierOne',
                      'CAD','ARMS','RMS','Spillman','Versaterm','OSSI','IMC','Mark43',
                      'CentralSquare','Tritech','Sun Ridge Systems')
    $installedNames = @(Get-ItemProperty `
        'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*',
        'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' `
        -ErrorAction SilentlyContinue | Select-Object -ExpandProperty DisplayName -ErrorAction SilentlyContinue)
    $cjisFound = $installedNames | Where-Object { $n = $_; $cjisSoftware | Where-Object { $n -match $_ } }
    if ($cjisFound) {
        Log-Info "CJIS-relevant software detected: $($cjisFound -join ', ')"
        Log-Info "--- CJIS Technical Security Controls Assessment ---"
        # Password policy
        if ($Script:MinPasswordLen -eq 0 -or $Script:MinPasswordLen -lt 8) {
            Log-Warn "  [CJIS] Password policy  -  minimum length $Script:MinPasswordLen (CJIS requires 8+ characters)"
        } else {
            Log-Info "  [CJIS] Password policy  -  minimum length $Script:MinPasswordLen (OK)"
        }
        # Encryption
        if ($bitlockerWarn) {
            Log-Warn "  [CJIS] Encryption  -  BitLocker OFF (CJIS Policy 5.10.1.1 requires encryption)"
        } else {
            Log-Info "  [CJIS] Encryption  -  BitLocker ON (OK)"
        }
        # Audit logging
        Log-Info "  [CJIS] Audit logging  -  process creation auditing enabled (OK)"
        # Inactive accounts
        if ($inactiveAccounts.Count -gt 0) {
            Log-Warn "  [CJIS] Access control  -  $($inactiveAccounts.Count) inactive account(s) (CJIS requires timely removal)"
        } else {
            Log-Info "  [CJIS] Access control  -  no inactive accounts detected (OK)"
        }
        # Windows Firewall
        $fwProfiles = @(Get-NetFirewallProfile -ErrorAction SilentlyContinue)
        $fwOff = @($fwProfiles | Where-Object { $_.Enabled -eq $false })
        if ($fwOff.Count -gt 0) {
            Log-Warn "  [CJIS] Network  -  Windows Firewall disabled on $($fwOff.Count) profile(s)"
        } else {
            Log-Info "  [CJIS] Network  -  Windows Firewall enabled on all profiles (OK)"
        }
    } else {
        Log-Info "CJIS check  -  no law enforcement software detected"
    }
} catch { Log-Info "CJIS check skipped  -  $($_.Exception.Message)" }

# Redirected folder scan - detect PUAs in non-C:\ user home directories
if ($Script:Config.ScanRedirectedFolders) {
    try {
        $redirectedDrives = @(Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue |
                              Where-Object { $_.Root -notmatch '^C:\\' -and (Test-Path "$($_.Root)Users") })
        if ($redirectedDrives.Count -gt 0) {
            Log-Info "Redirected folder scan  -  $($redirectedDrives.Count) non-C:\ drive(s) with Users folder found"
            $puaExts = @('.exe','.bat','.cmd','.vbs','.js','.ps1','.msi','.scr')
            $scanSubFolders = @('Downloads','Desktop','AppData\Local\Temp','AppData\Roaming\Temp')
            ...
            foreach ($drive in $redirectedDrives) {
                ...
                foreach ($userFolder in $userFolders) {
                    # Extract username from deepest meaningful folder
                    $userName = $userFolder.Name
                    foreach ($subFolder in $scanSubFolders) {
                        $scanPath = Join-Path $userFolder.FullName $subFolder
                        if (-not (Test-Path -LiteralPath $scanPath)) { continue }
                        $suspFiles = @(Get-ChildItem -LiteralPath $scanPath -Force -File `
                                       -ErrorAction SilentlyContinue |
                                       Where-Object { $puaExts -contains $_.Extension.ToLower() })
                        foreach ($f in $suspFiles) {
                            # Check against PUP filename IOC list
                            $isPUA = $Script:FilenameIOCs -and ($Script:FilenameIOCs | Where-Object { $f.Name -match $_ })
                            # Check against known PUP patterns
                            $puaPattern = [System.Text.RegularExpressions.Regex]::new(
                                'toolbar|hijack|adware|pup|bundl|install.*setup|setup.*install|' +
                                ('news'+'setup')+'|crawler|conduit|babylon|sweet.?im',
                                [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
                            $matchesPUA = $puaPattern.IsMatch($f.Name)
                            if ($isPUA -or $matchesPUA) {
                                Log-IOC "PUA in redirected folder  -  User: $userName  -  File: $($f.Name)"
                                ...
                                try {
                                    ...
                                    Log-Fail "Could not remove: $($f.FullName)  -  $($_.Exception.Message)"
                                }
                            }
                        }
                    }
            ...
            Log-Info "Redirected folder scan  -  no non-C:\\ drives with Users folders found"
        }
    } catch { Log-Info "Redirected folder scan skipped  -  $($_.Exception.Message)" }
}

# Remote Access Tool Inventory
if ($Script:Config.RemoteAccessInventory) {
    try {
        Log-Info '--- Remote Access Tool Inventory ---'
        ...
        # Known RMM-bundled VNC paths - these are legitimate even without ARP entry
        $legitimateVNCPaths = @(
            'CentraStage',      # Datto RMM bundles UltraVNC
            'Kaseya',           # Kaseya VSA bundles VNC
            'LabTech',          # ConnectWise Automate bundles VNC
            'ConnectWise',      # ConnectWise bundles VNC
            'NinjaRMM',         # NinjaRMM bundles VNC
            'Atera',            # Atera bundles VNC
            'N-able'            # N-able bundles VNC
        )

        $arpList = @(Get-ItemProperty `
            'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*',
            'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' `
            -ErrorAction SilentlyContinue | Select-Object DisplayName, InstallLocation, DisplayVersion)
        ...
        foreach ($tool in $remoteTools) {
            ...
            $svcs = @()
            foreach ($svcPat in $tool.Services) {
                $svcs += @(Get-Service -ErrorAction SilentlyContinue |
                           Where-Object { $_.Name -like $svcPat -or $_.DisplayName -like $svcPat })
            }
            if ($svcs.Count -gt 0) {
                $found = $true
                ...
                if ($tool.Name -eq 'ScreenConnect') {
                    foreach ($svc in $svcs) {
                        if ($svc.DisplayName -match '\(([a-f0-9\-]{8,})\)') { $instanceIDs += $Matches[1] }
                    }
                }
            }

            $procs = @()
            foreach ($procPat in $tool.Procs) {
                $procs += @(Get-Process -ErrorAction SilentlyContinue |
                            Where-Object { $_.Name -like $procPat })
            }
            if ($procs.Count -gt 0) {
                $found = $true
                $procStatus = "Running (PID: $(($procs.Id) -join ','))"
            }

            $arpMatch = @($arpList | Where-Object { $_.DisplayName -match $tool.ARP })
            ...
            if ($tool.Tier -eq 'RAT') {
                # Check if this is an RMM-bundled VNC - legitimate even without ARP
                $isRMMBundled = $false
                if ($svcs.Count -gt 0) {
                    foreach ($svc in $svcs) {
                        try {
                            ...
                            if ($svcWmi -and $svcWmi.PathName) {
                                $bundleMatch = $legitimateVNCPaths | Where-Object { $svcWmi.PathName -match [regex]::Escape($_) }
                                if ($bundleMatch) { $isRMMBundled = $true; break }
                            }
                        } catch { }
                    }
                }
                ...
                if ($isRMMBundled) {
                    Log-Info "[REMOTE ACCESS]  $($tool.Name)  -  RMM-bundled component (legitimate)"
                    Log-Info "  Service: $svcStatus  |  Path confirms RMM installation"
                } else {
                    Log-IOC "[RISKWARE-RAT]  $($tool.Name) detected  -  IMMEDIATE REVIEW REQUIRED"
                    Log-IOC "  Service: $svcStatus  |  Process: $procStatus  |  In ARP: $(if ($inARP) { 'Yes' } else { 'No' })"
                    Log-IOC "  Actively exploited in ransomware attack chains  -  verify with client immediately"
                    $Script:Counters.IOCsFound++
                }
            } else {
                if (-not $inARP -and $Script:Config.RemoteAccessWarnUnknown) {
                    ...
                    Log-Info "[REMOTE ACCESS]  $($tool.Name)$idStr  -  In ARP: $(if ($inARP) { 'Yes' } else { 'No' })  |  Ver: $version"
        ...
        if ($foundTools.Count -eq 0) {
            Log-Info "Remote access inventory  -  no remote access tools detected"
        } else {
            Log-Info "Remote access inventory complete  -  $($foundTools.Count) tool(s): $($foundTools -join ', ')"
        }
    } catch { Log-Info "Remote access inventory skipped  -  $($_.Exception.Message)" }
}
...
try {
    ...
    # CIS 1.1.1 - Password minimum length >= 14 (Level 1 = 8, Level 2 = 14)
    try {
        ...
        if ($minLen -and [int]$minLen -lt 8) {
            Log-Warn "  [CIS 1.1.1] Password minimum length is $minLen  -  recommend 8+ (Level 1) or 14+ (Level 2)"
            $cisIssues++
        } else { Log-Info "  [CIS 1.1.1] Password minimum length: $minLen (OK)" }
    } catch { }

    # CIS 2.2 - Guest account disabled
    try {
        $guest = Get-LocalUser -Name 'Guest' -ErrorAction SilentlyContinue
        if ($guest -and $guest.Enabled) {
            Log-Warn "  [CIS 2.2] Guest account is ENABLED  -  should be disabled"
            $cisIssues++
        } else { Log-Info "  [CIS 2.2] Guest account disabled (OK)" }
    } catch { }

    # CIS 2.3 - LAN Manager authentication level
    try {
        $lmAuth = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' `
                   -Name 'LmCompatibilityLevel' -ErrorAction SilentlyContinue).LmCompatibilityLevel
        if ($null -eq $lmAuth -or $lmAuth -lt 3) {
            Log-Warn "  [CIS 2.3] LAN Manager auth level is $lmAuth  -  recommend 5 (NTLMv2 only)"
            $cisIssues++
        } else { Log-Info "  [CIS 2.3] LAN Manager auth level: $lmAuth (OK)" }
    } catch { }

    # CIS 2.4 - Anonymous SID enumeration disabled
    try {
        $anonSid = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' `
                    -Name 'RestrictAnonymousSAM' -ErrorAction SilentlyContinue).RestrictAnonymousSAM
        if ($anonSid -ne 1) {
            Log-Warn "  [CIS 2.4] Anonymous SAM enumeration not restricted  -  recommend setting to 1"
            $cisIssues++
        } else { Log-Info "  [CIS 2.4] Anonymous SAM enumeration restricted (OK)" }
    } catch { }

    # CIS 2.5 - Windows Firewall enabled on all profiles
    try {
        $fwProfiles = @(Get-NetFirewallProfile -ErrorAction Stop)
        $fwOff = @($fwProfiles | Where-Object { $_.Enabled -eq $false })
        if ($fwOff.Count -gt 0) {
            Log-Warn "  [CIS 2.5] Windows Firewall disabled on: $($fwOff.Profile -join ', ')"
            $cisIssues++
        } else { Log-Info "  [CIS 2.5] Windows Firewall enabled on all profiles (OK)" }
    } catch { }

    # CIS 2.6 - SMBv1 disabled
    try {
        $smb1 = Get-SmbServerConfiguration -ErrorAction Stop | Select-Object -ExpandProperty EnableSMB1Protocol
        if ($smb1) {
            Log-Warn "  [CIS 2.6] SMBv1 is ENABLED  -  should be disabled (critical)"
            $cisIssues++
        } else { Log-Info "  [CIS 2.6] SMBv1 disabled (OK)" }
    } catch { }

    # CIS 2.7 - Remote Registry disabled
    try {
        $remReg = Get-Service -Name 'RemoteRegistry' -ErrorAction SilentlyContinue
        if ($remReg -and $remReg.StartType -ne 'Disabled') {
            Log-Warn "  [CIS 2.7] Remote Registry service is $($remReg.StartType)  -  recommend Disabled"
            $cisIssues++
        } else { Log-Info "  [CIS 2.7] Remote Registry disabled (OK)" }
    } catch { }

    # CIS 2.8 - AutoRun/AutoPlay disabled
    try {
        ...
        if ($autoRun -ne 255) {
            Log-Warn "  [CIS 2.8] AutoRun/AutoPlay not fully disabled  -  recommend NoDriveTypeAutoRun=255"
            $cisIssues++
        } else { Log-Info "  [CIS 2.8] AutoRun/AutoPlay disabled (OK)" }
    } catch { }

    # CIS 2.9 - Windows Defender enabled
    try {
        $mpStatus = Get-MpComputerStatus -ErrorAction Stop
        if (-not $mpStatus.AMServiceEnabled) {
            Log-Warn "  [CIS 2.9] Windows Defender service not enabled"
            $cisIssues++
        } else { Log-Info "  [CIS 2.9] Windows Defender service enabled (OK)" }
    } catch { Log-Info "  [CIS 2.9] Defender status unavailable" }

    if ($cisIssues -eq 0) {
        Log-Info "CIS Benchmark Lite  -  all Level 1 checks passed"
    } else {
        Log-Warn "CIS Benchmark Lite  -  $cisIssues check(s) failed  -  review above"
    }
} catch { Log-Info "CIS Benchmark check skipped  -  $($_.Exception.Message)" }
if ($Script:Config.MagicBytesAudit -and $Script:Config.ScanDepth -eq 'Deep') {
    try {
        $magicSigs = @{
            'MZ'     = @('.exe','.dll','.sys','.ocx')
            '%PDF'   = @('.pdf')
            'PK'     = @('.zip','.docx','.xlsx','.pptx','.jar')
            'Rar!'   = @('.rar')
            '7z'     = @('.7z')
        }
        $checkExts = @('.exe','.dll','.docx','.xlsx','.pdf','.zip','.js','.vbs','.ps1','.bat','.cmd')
        $scanStart = Get-Date
        $scanned   = 0
        $mismatches = @()

        foreach ($scanPattern in $Script:Config.MagicBytesScanPaths) {
            $expanded = @(Resolve-Path $scanPattern -ErrorAction SilentlyContinue)
            foreach ($path in $expanded) {
                if (-not (Test-Path -LiteralPath $path)) { continue }
                $files = @(Get-ChildItem -LiteralPath $path -Recurse -Force -File `
                           -ErrorAction SilentlyContinue |
                           Where-Object { $checkExts -contains $_.Extension.ToLower() } |
                           Select-Object -First ($Script:Config.MagicBytesMaxFiles - $scanned))
                foreach ($f in $files) {
                    if ($scanned -ge $Script:Config.MagicBytesMaxFiles) { break }
                    if (((Get-Date) - $scanStart).TotalSeconds -gt 30) { break }
                    try {
                        $bytes = [System.IO.File]::ReadAllBytes($f.FullName) | Select-Object -First 8
                        $header = [System.Text.Encoding]::ASCII.GetString($bytes).Substring(0,[Math]::Min(4,$bytes.Count))
                        foreach ($sig in $magicSigs.Keys) {
                            if ($header.StartsWith($sig)) {
                                if ($magicSigs[$sig] -notcontains $f.Extension.ToLower()) {
                                    $mismatches += "$($f.Extension) file with $sig header: $($f.FullName)"
                                }
                                break
                            }
                        }
        ...
        if ($mismatches.Count -gt 0) {
            Log-IOC "Magic bytes mismatch  -  $($mismatches.Count) file(s) with mismatched extension/content:"
            foreach ($m in $mismatches) { Log-IOC "  $m" }
            $Script:Counters.IOCsFound += $mismatches.Count
        } else {
            Log-Info "Magic bytes audit  -  $scanned files scanned, no mismatches found"
        }
    } catch { Log-Info "Magic bytes audit skipped  -  $($_.Exception.Message)" }
} elseif ($Script:Config.MagicBytesAudit) {
    Log-Info "Magic bytes audit  -  requires Deep scan tier (current: $($Script:Config.ScanDepth))"
}
...
# SMBv1 scoring
try {
    $smb1Score = Get-SmbServerConfiguration -ErrorAction Stop | Select-Object -ExpandProperty EnableSMB1Protocol
    if ($smb1Score) { $secScore -= 20 }
} catch { }

# LAN Manager auth level scoring
try {
    $lmLevel = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' `
                -Name 'LmCompatibilityLevel' -ErrorAction SilentlyContinue).LmCompatibilityLevel
    if ($null -eq $lmLevel -or $lmLevel -lt 3) { $secScore -= 15 }
} catch { }

# Windows Firewall scoring
try {
    $fwDisabled = @(Get-NetFirewallProfile -ErrorAction Stop | Where-Object { $_.Enabled -eq $false })
    if ($fwDisabled.Count -gt 0) { $secScore -= 15 }
} catch { }

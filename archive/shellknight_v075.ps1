...
<#
.SYNOPSIS
    ShellKnight v0.75  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.75  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($profile in $userProfiles) {
        ...
        $scFolders = @(Get-ChildItem -LiteralPath $clickOncePath -Directory -Recurse -Force `
                       -ErrorAction SilentlyContinue |
                       Where-Object { $_.Name -match 'screenconnect|scre' })

        foreach ($scFolder in $scFolders) {
            # Extract instance ID from folder contents
            $clientExe = @(Get-ChildItem -LiteralPath $scFolder.FullName -Filter '*.exe' `
...
try {
    ...
    $staleProfileExclusions = @(
        ...
        'defaultuser0', 'defaultuser1',     # Windows imaging artifacts
        'QBDataServiceUser30', 'QBDataServiceUser31', 'QBDataServiceUser32',
        'QBDataServiceUser33', 'QBDataServiceUser34', 'QBDataServiceUser35'  # QuickBooks service accounts
    )
...
if ($totalIssues -eq 0) {
    ...
    $issueText  = "   ShellKnight: Action Required  -  $totalIssues issue(s) detected. Review report above."
    $issueLine  = $issueText.PadRight(74)

...
<#
.SYNOPSIS
    ShellKnight v0.74  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.74  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
Write-Host ''
if ($totalIssues -eq 0) {
    $clearStr = 'ShellKnight: All Clear!'
    $clearPad = ' ' * ([Math]::Max(0, 74 - $clearStr.Length))
    Write-Host ('  ' + ('#' * 76)) -ForegroundColor Green
    Write-Host ('  #' + (' ' * 74) + '#') -ForegroundColor Green
    Write-Host "  #   $clearStr$clearPad#" -ForegroundColor Green
    Write-Host ('  #' + (' ' * 74) + '#') -ForegroundColor Green
    Write-Host ('  ' + ('#' * 76)) -ForegroundColor Green
} else {
    $issueStr = "ShellKnight: Action Required  -  $totalIssues issue(s) detected. Review report above."

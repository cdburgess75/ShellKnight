...
<#
.SYNOPSIS
    ShellKnight v0.72  -  Enterprise Endpoint Security & Remediation Tool
...
Write-Host ("  ShellKnight v0.72  |  $env:COMPUTERNAME  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')  |  PS $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)") -ForegroundColor Cyan
Write-Host ("  " + ("=" * 78)) -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    foreach ($scanPath in $scanPaths) {
        ...
        $hits = @(Get-ChildItem -LiteralPath $scanPath -Recurse -Force -File -ErrorAction SilentlyContinue |
                  Where-Object { $ransomPattern.IsMatch($_.Name) } |
